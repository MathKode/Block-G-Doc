document.getElementById('disable-button').addEventListener('click', function() {
    // Enregistrer le temps actuel + 1heure dans le stockage local
    // Si le timer existe déjà, on y ajoute 1heure
    time = chrome.storage.local.get('disableUntil', function(data) {
        if (Date.now() < data.disableUntil) {
            const disableUntil = data.disableUntil + 3600000; // 1 heure en millisecondes
...
function timeleft() {
    ...
    if (data.disableUntil) {
        ...
        if (millisecond < 0) {
            ...
            minute_and_hour = Math.floor((millisecond % (1000 * 60 * 60 * 60)) / (1000*60));
            if (second < 10) { second = '0' + second; }
            if (minute_and_hour < 10) { minute_and_hour = '0' + minute_and_hour; }
            document.getElementById('seconds').textContent = second;
            document.getElementById('minutes').textContent = minute_and_hour;
